...
import { Bell, CircleDollarSign, Search, ShoppingBag, User } from "lucide-react";
...
export function CommandPalette({
  ...
  const allowedPages = new Set<ActivePanel>(pageItems.map(([id]) => id));
  const marketViewAllowed = () => allowedPages.has("market");
  const commands = [
    ...
    { key: "price-finder", label: "Market Browse", description: "Find global listings and price history", icon: <CircleDollarSign size={15} />, allowed: marketViewAllowed(), locked: false, run: () => onNavigate("market", "browse") },
    { key: "deal-watchlist", label: "Deal Watch", description: "Manage watched market deals", icon: <Bell size={15} />, allowed: marketViewAllowed(), locked: false, run: () => onNavigate("market", "deal-watch") },
    { key: "buy-order-finder", label: "Buy Orders", description: "Find active global buy orders", icon: <ShoppingBag size={15} />, allowed: marketViewAllowed(), locked: false, run: () => onNavigate("market", "buy-orders") },
