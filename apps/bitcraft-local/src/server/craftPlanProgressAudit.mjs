...
export function createCraftPlanProgressAuditRepository(db, {
  ...
  function stateFor(claimId, planId) {
  ...
  function latestSuccess(claimId, planId) {
  ...
  function recordSuccess(snapshot) {
    ...
    const planId = text(snapshot?.planId);
    const capturedAt = text(snapshot?.capturedAt || now());
    if (!claimId || !planId) throw new Error("Craft Plan progress audit snapshot requires claim and plan IDs.");
    const state = stateFor(claimId, planId);
  ...
  function recordFailure(claimId, planId, failures, capturedAt = now()) {
  ...
  function listEvents(claimId, planId, {
  ...
  function status(claimId, planId) {
    const state = stateFor(claimId, planId);
  ...
  function exportRange(claimId, planId, range) {
    ...
    return {
      ...
      snapshots,
      events: listEvents(claimId, planId, { since: effectiveSince, limit: 10_000 }),
      warnings,
    };
  }
