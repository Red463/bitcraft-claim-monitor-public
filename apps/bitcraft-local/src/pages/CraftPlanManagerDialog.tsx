...
function emptyConfig(): CraftPlanConfig {
  return { enabled: true, name: "Claim craft plan", targets: [], sourceRules: { storageContainerIds: [], playerIds: [], craftPlayerIds: [], bankPlayerIds: [], deployableContainerIds: [] }, routeOverrides: {}, sectionOverrides: {}, rowNameOverrides: {}, multipliers: {}, gatheredItemKeys: [], buildingProgress: {} };
}
    ...
    <Dialog open title="Craft plan manager" closeOnBackdrop={false} onClose={onClose} className="modal craft-plan-manager" backdropClassName="modal-backdrop craft-plan-manager-backdrop">
        <header className="modal-header">
          <div>
            ...
            <p>{sharedMode ? "Manage this shared claim plan. Only browsers holding its private edit key, or an administrator, can save changes." : "Set goals, choose counted inventories, apply tier presets, and tune routes for the public Craft Planning board."}</p>
          </div>
        ...
        <div className="craft-plan-manager-body" aria-busy={busy || catalogBusy}>
          ...
          {activeTab === "targets" ? <section className="craft-plan-manager-panel">
            ...
            <section className="craft-plan-tier-presets" aria-label="Tier upgrade presets">
              ...
              </div> : <div className="craft-plan-preset-empty"><strong>No tier presets loaded</strong><span>BitJita did not return tier upgrade research materials for this claim.</span></div>}
            </section>
