...
import { ArrowLeft, Castle, Clock, Crown, Landmark, MapPin, Package, Users, X } from "lucide-react";
...
import { dateLabel, formatGoldAmount, formatNumber, timeAgo } from "../../utils/format";

type EmpireDetailsTab = "overview" | "members" | "claims";
type EmpireDetailsState = { data: AnyRecord | null; loading: boolean; error: string | null };
...
function coordinateText(row: AnyRecord) {
  const x = row.locationX ?? row.x;
  const z = row.locationZ ?? row.z;
  return x == null || z == null ? "Location unavailable" : `${formatNumber(Number(x), 0)}, ${formatNumber(Number(z), 0)}`;
}
...
export function EmpireDetailsDialog({ empireId, regionId, inactiveDays, onClose, onBack }: EmpireDetailsDialogProps) {
  ...
  React.useEffect(() => setTab("overview"), [empireId]);
  React.useEffect(() => {
    ...
    const refresh = fetch(`/api/local/empires/details?${params}`, {
    ...
    void trackPromise("empire-details", refresh).catch((error) => {
  ...
  return (
    <Dialog open title={String(empire.name ?? "Empire Details")} description="Empire activity, members, and claim overview." onClose={onClose} className="help-dialog empire-details-dialog">
      ...
      <div className="empire-detail-body">
        {onBack ? <button type="button" className="toolbar-button empire-detail-back" onClick={onBack}><ArrowLeft size={14} /> Back</button> : null}
        {state.loading && !data ? <AppSkeleton /> : null}
        {state.error && !data ? <AsyncState kind="error" title="Unable to load empire details" detail={state.error} action={<button type="button" className="toolbar-button" onClick={() => setRetry((value) => value + 1)}>Retry</button>} /> : null}
        {data ? (
          <>
            ...
            {data.partial || errors.length ? <div className="warning-card">Some empire sources are unavailable: {errors.join("; ")}</div> : null}
            ...
            <div className="empire-detail-tabs" role="tablist" aria-label="Empire detail views">
              {tabs.map((entry) => (
                <button key={entry.id} id={`empire-detail-tab-${entry.id}`} type="button" role="tab" aria-selected={tab === entry.id} aria-controls={`empire-detail-panel-${entry.id}`} className={tab === entry.id ? "active" : ""} onClick={() => setTab(entry.id)}>
                  {entry.label}{entry.count == null ? null : <small>{formatNumber(entry.count)}</small>}
                </button>
              ))}
            </div>
            <section id={`empire-detail-panel-${tab}`} className="empire-detail-panel" role="tabpanel" aria-labelledby={`empire-detail-tab-${tab}`}>
              {tab === "overview" ? (
              ...
              {tab === "members" ? (
                members.length ? <div className="empire-detail-list">{members.map((member) => (
                  <article key={member.entityId ?? member.username}>
                    <div><strong>{member.username ?? "Unknown"}</strong><small>{member.rankTitle ?? "Citizen"}</small>{member.hasStorage ? <div className="empire-detail-flags"><span><Package size={12} /> Storage</span></div> : null}</div>
                    <span>{member.signedIn ? "Online now" : compactDate(member.lastLoginTimestamp)}</span>
                  </article>
                ))}</div> : <AsyncState kind="empty" title="No current member data available" detail="BitJita did not return members for this empire." compact />
              ) : null}
              {tab === "claims" ? (
                claims.length ? <div className="empire-detail-list">{claims.map((claim) => (
                  <article key={claim.claimId ?? claim.name}>
                    <div><strong>{claim.name ?? "Unknown claim"}</strong><small>{claim.ownerName ?? "Unknown owner"} · {coordinateText(claim)}</small></div>
                    <span>T{claim.tier ?? "?"} · {formatNumber(claim.supplies)} supplies · {formatGoldAmount(claim.treasury)}</span>
                  </article>
                ))}</div> : <AsyncState kind="empty" title="No current claim data available" detail="No regional claims are associated with this empire." compact />
              ) : null}
            </section>
          </>
        ) : null}
